...
const RecordVoice = ({
  ...
  const playRecord = async () => {
    ...
    audioRecorderPlayer.addPlayBackListener(e => {
      ...
      const _duration = audioRecorderPlayer.mmss(Math.floor(e.duration / 1000));
  ...
  const pausePlay = async () => {
    try {
      await audioRecorderPlayer.pausePlayer().catch(err => {
  ...
  const stopPlay = async () => {
    await audioRecorderPlayer.stopPlayer().catch(err => {
    ...
    <SafeAreaView style={styles.container}>
      ...
      <View style={styles.recordBox}>
        {recordStatus === 0 ? (
          ...
          /* (recordStatus === 3 || recordStatus === 4) */ <Text
      ...
      <View style={styles.buttonBox}>
        <SmallWhiteButton onPress={cancelRecord} text="취소" />
        <SmallButton
          onPress={() => {
            onSubmit().catch(err => {
