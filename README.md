# 🍎사과나무 추억걸렸네🍏

![그래픽이미지 (1)](exec/img/그래픽이미지.png)

<p align="center">🎁 <a href="https://play.google.com/store/apps/details?id=com.cotyledon.appletree"> 어플 다운받기 </a> 🎁 </p>

<br>


# 📰 프로젝트 개요

**친구들과 재미있게 타임캡슐을 만들어 보세요!**
<br> <br>
추억에는 그때의 생각과 메시지가 있기 마련입니다. <br> 사과에 소중한 추억을 담아 풍성한 사과나무를 만들어 보세요. <br> 약속된 시간이 되면 친구들과 같이 사과를 따서 추억을 꺼내어 보아요.

<br>

# 🗓️ 개발기간

**2022.10.11 ~ 2022.11.18 (6주)**

<br>

# 📽️ UCC

[UCC 바로가기](https://www.youtube.com/watch?v=KiL8vpJiV0E)   
<img src="/exec/img/ucc-cover.png" width="300"/>

<br>

# 🌟 Team

|     Name     |                      김낙현                      |                     송선아                      |                     송제영                      |                     이예은                      |                   조다연                   |                     차송희                      |
| :----------: | :----------------------------------------------: | :---------------------------------------------: | :---------------------------------------------: | :---------------------------------------------: | :----------------------------------------: | :---------------------------------------------: |
| **Profile**  | <img width="145" src="exec/img/Group 207.png"/>  | <img width="150" src="exec/img/Group 209.png"/> | <img width="140" src="exec/img/Group 206.png"/> | <img width="150" src="exec/img/Group 208.png"/> | <img width="150" src="exec/img/yeon.png"/> | <img width="145" src="exec/img/Group 211.png"/> |
| **Position** |               Infra<br />FullStack               |                    FullStack                    |            Team Leader<br />Backend             |                    FullStack                    |            Frontend<br /> UI/UX            |          Frontend<br />UI/UX<br />UCC           |
|   **Git**    | [@Psalmist-KIM](https://github.com/Psalmist-KIM) |     [@Seona98](https://github.com/seona98)      |   [@hooreique](https://github.com/hooreique)    |       [@lye2i](https://github.com/lye2i)        |   [@dus6982](https://github.com/dus6982)   |  [@chasonghui](https://github.com/chasonghui)   |

<br>

# 🛠️ 기술스택

| Tech         | Stack                                        |
| ------------ | -------------------------------------------- |
| **Backend**  | Java, Spring Boot, JPA, RabbitMQ             |
| **Frontend** | React-Native                                 |
| **DataBase** | MySQL, Redis                                 |
| **Tools**    | AWS EC2, Firebase Auth, Firebase Storage, NginX, Docker, GitLab, JIRA, Notion |

<br>

# 📌 시스템 아키텍처

![아키텍처](exec/img/아키텍처.png)

<br>

## 🗃️ ERD

![ERD](exec/img/ERD.png)

<br>

# 🔎 주요 기능

### 1. 실행화면

- 인트로 및 로그인

  <img src="exec/gif/처음화면.gif" width="250">


### 2. 메인 화면

|                사과가 없는 경우                 |                사과가 있는 경우                 |                  잠긴사과 모달                  |
| :---------------------------------------------: | :---------------------------------------------: | :---------------------------------------------: |
| ![사과없는경우](exec/gif/메인-사과없는경우.gif) | ![사과있는경우](exec/gif/메인-사과있는경우.gif) | ![잠긴사과모달](exec/gif/메인-잠긴사과모달.gif) |

### 3. 사과 만들기

- 세션 생성 및 입장
  | 세션 생성 | 세션 입장 | 세션 목록 |
  | :---------------------------------------------------------------------------------------: | :---------------------------------------------------------------------: | :----------------------------------------------------------------------------------------------------------: |
  | ![](exec/gif/세션생성.gif) | ![세션입장](exec/gif/세션입장.gif) | ![세션목록](exec/gif/세션목록.gif) |

- DB에 사과 넣기
  | 사과에 추억 담기 | 녹음폼 |
  | :---------------------------------------------------------------------------------------: | :---------------------------------------------------------------------: |
  | ![사과에 추억 담기](exec/gif/사과에추억담기.gif) | ![녹음폼](exec/gif/녹음폼.gif) |

### 4. 사과 리스트 보기

| 사과가 없는 경우                           | 열린 사과 목록                             | 잠김 사과 목록                             |
| ------------------------------------------ | ------------------------------------------ | ------------------------------------------ |
| ![사과없는경우](exec/gif/사과없는경우.gif) | ![열린사과목록](exec/gif/열린사과목록.gif) | ![잠긴사과목록](exec/gif/잠긴사과목록.gif) |

### 5. 사과 따기

- 세션 입장 (사과 때리기)
  <br><img src="exec/gif/사과때리기.gif" width="250">

### 6. 사과 상세보기

- 잠긴사과 상세보기
  <br>
  <img src="exec/gif/잠긴사과상세보기.gif" width="250">

- 열린사과 상세보기
  | 열린사과 상세보기 | 씨앗 상세보기 |
  | :---------------------------------------------------------------------------------------: | :---------------------------------------------------------------------------------------------------------: |
  | ![열린사과상세보기](exec/gif/열린사과상세보기.gif) | ![씨앗상세보기](exec/gif/씨앗상세보기.gif) |

### 7. 이메일 전송

- 이메일 확인

  <img src="exec/img/메일.png" width="250">

### 8. 지도 보기

- 만든 사과 위치 표시

    <img src="exec/gif/지도보기.gif" width="250">

### 9. 마이 페이지

- 마이페이지

  <img src="exec/img/마이페이지.png" width="250">


<br>

## 시연영상

[시연영상 바로가기](https://www.youtube.com/watch?v=9pNwGHzRI6g)

<br>

## 🔗 WIKI

📖 [Team Notion](https://chasonghui.notion.site/1202c1502e9b410ea561ee25ee6ac659)

🙌 [Git Convention](https://chasonghui.notion.site/commit-branch-6119490deff242cc9c4afbf3bbe61aa3)

📜 [API 명세서](https://chasonghui.notion.site/API-f71b36093160458694ac36cb2cf62ed1) | [FIGMA](https://www.figma.com/file/0xazrudv5SUGJREIuPYF4V/%EC%82%AC%EA%B3%BC%EB%82%98%EB%AC%B4%EC%B6%94%EC%96%B5%EA%B1%B8%EB%A0%B8%EB%84%A4?node-id=0%3A1)

<br>
